...
public interface RecipeRepository extends CrudRepository<Recipe, Long> {
    @Query(value = "SELECT DISTINCT r.* FROM recipe r" +
            ...
            "r.portions <= CASE WHEN ?2 > 0 THEN ?2 ELSE 12", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT r.* FROM recipe r" +
            ...
            "r.portions <= CASE WHEN ?3 > 0 THEN ?3 ELSE 12", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT r.* FROM recipe r" +
            ...
            "r.portions <= CASE WHEN ?4 > 0 THEN ?4 ELSE 12", nativeQuery = true)
